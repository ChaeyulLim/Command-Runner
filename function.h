...
std::wstring CommandRun(std::wstring command)
{
    ...
    if (command == L"백준 과제" || command == L"baekjoon") {
        ShellExecuteW(NULL, L"open", L"msedge", L"--new-window https://github.com/ChaeyulLim", NULL, SW_SHOWNORMAL);
        ShellExecuteW(NULL, L"open", L"msedge", L"-new https://www.acmicpc.net/step", NULL, SW_SHOWNORMAL);
